```
TITLE: GIB Games Universal Style Guide (KJU Edition)
...
<a name="versioning"></a>
### 2.5 Versioning
GIB Games projects use [Semantic Versioning](https://www.geeksforgeeks.org/introduction-semantic-versioning/).
...
This versioning is detected by the build system using [Git tags](#gittags).
...
Make use of Odin Inspector's [Infobox Attribute](https://odininspector.com/attributes/info-box-attribute) when necessary.
...
For each build release, the commit of that build should have a git tag added in the format `v#.#`. This tag is used for [build versioning](#versioning).
